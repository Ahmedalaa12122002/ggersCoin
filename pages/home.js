/* =====================================================
   WinHive Farm – Home Page (Visual Enhanced)
===================================================== */
...
function plotStatus(plot) {
  ...
  if (elapsed > plot.growTime * 0.65) {
    return { stage: "growing2", remaining: remain };
  }
...
window.openPlantMenu = function (index) {
  ...
  CROPS.forEach(c => {
    menu += `
      <button onclick="plant(${index},'${c.id}')"
        style="margin:6px;padding:8px 14px;border:none;
        border-radius:10px;background:#222;color:#fff">
        ${c.icon} ${c.name}
      </button>`;
  });
  ...
  menu += `
      <br><br>
      <button onclick="closePlantMenu()" style="background:none;border:none;color:#aaa">
        إلغاء
      </button>
    </div>
  </div>`;
